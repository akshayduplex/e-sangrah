...
router.get("/register", authenticate, checkPermissions, async (req, res) => {
    try {
        ...
        res.render("pages/donor-registration", { title: "E-Sangrah - Register", donor: null, isEdit: false, user: req.user });
    }
});
...
router.get("/list", authenticate, checkPermissions, async (req, res) => {
    try {
        ...
        res.render("pages/donor-listing", { title: "E-Sangrah - Donor List", donors, user: req.user });
    } catch (err) {
