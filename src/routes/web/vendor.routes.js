...
router.get("/register", authenticate, checkPermissions, async (req, res) => {
    try {
        ...
        res.render("pages/vendor-registration", { title: "E-Sangrah - Register", vendor: null, isEdit: false, user: req.user });
    }
});
...
router.get("/list", authenticate, checkPermissions, async (req, res) => {
    try {
        ...
        res.render("pages/vendor-registration-list", { title: "E-Sangrah - Vendor List", vendors, user: req.user });
    } catch (err) {
