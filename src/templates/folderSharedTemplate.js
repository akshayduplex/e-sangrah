...
export const folderSharedTemplate = (data) => {
    const {
        userName = '',
        senderName = '',
        folderName = '',
    ...
    html += '<title>Folder Shared With You</title>';
    html += '</head>';
    ...
    html += `<h2 style="font-size:24px; color:#fff; font-weight:500; margin:0; padding:15px 0;">Folder Shared With You</h2>`;
    ...
    html += `<p style="font-size:15px; line-height:25px;">Hi ${userName || 'there'},</p>`;
    html += `<p style="font-size:15px; line-height:25px;">${senderName || 'Someone'} has shared a folder with you.</p>`;
    ...
    html += `<strong>Folder Name:</strong> ${folderName || 'Unnamed Folder'}<br>`;
    html += `<strong>Access Level:</strong> ${access || 'N/A'}<br>`;
    html += `<strong>Expires At:</strong> ${formattedExpiry}`;
    html += `</p>`;
    html += `</div>`;

    html += `<div style="text-align:center; margin:25px 0;">`;
    html += `<a href="${folderLink}" target="_blank" rel="noopener noreferrer" style="background:${themeColor}; color:#fff; text-decoration:none; font-size:15px; padding:12px 25px; border-radius:6px; display:inline-block;">View Folder</a>`;
    html += `</div>`;

    html += `<p style="font-size:14px; color:#666;">You can now view or manage this folder as per your granted permissions.</p>`;
    html += `</td>`;
    html += `</tr>`;
